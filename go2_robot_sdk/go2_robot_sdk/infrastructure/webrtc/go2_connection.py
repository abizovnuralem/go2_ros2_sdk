...
_LIDAR_DECODE_LOCK = threading.Lock()
...
class Go2Connection:
    ...
    def __init__(
        ...
        self._pending_lidar_bytes: Optional[bytes] = None
        self._lidar_decode_task: Optional[asyncio.Task] = None
        ...
        try:
            ...
            if isinstance(message, str):
                ...
                if self.decode_lidar:
                    self._pending_lidar_bytes = message
                    if (
                        self._lidar_decode_task is None
                        or self._lidar_decode_task.done()
                    ):
                        try:
                            loop = asyncio.get_running_loop()
                            self._lidar_decode_task = loop.create_task(
                                self._decode_latest_lidar_bytes()
                            )
                        except RuntimeError:
                            msgobj = legacy_deal_array_buffer(message, True)
                else:
                    msgobj = legacy_deal_array_buffer(message, False)
    ...
    async def _decode_latest_lidar_bytes(self) -> None:
        while self._pending_lidar_bytes is not None:
            data = self._pending_lidar_bytes
            self._pending_lidar_bytes = None

            loop = asyncio.get_running_loop()
            msgobj = await loop.run_in_executor(
                None, self._decode_lidar_bytes_threadsafe, data
            )

            if self.on_message and msgobj is not None:
                self.on_message(data, msgobj, self.robot_num)

            await asyncio.sleep(0)

    def _decode_lidar_bytes_threadsafe(self, data: bytes):
        with _LIDAR_DECODE_LOCK:
            return legacy_deal_array_buffer(data, True)
